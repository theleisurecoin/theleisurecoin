...
// theleisurecoin: optional setting to unlock wallet for block minting only;
...
// theleisurecoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // theleisurecoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // theleisurecoin: sub-cent change is moved to fee
...
// theleisurecoin: create coin stake transaction
...
// theleisurecoin: check 'spent' consistency between wallet and txindex
// theleisurecoin: fix wallet spent state according to txindex
...
// theleisurecoin: disable transaction (only for coinstake)
